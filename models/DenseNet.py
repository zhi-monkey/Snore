...
class DenseNet(torch.nn.Module):
    def __init__(self,layer_num=(6,12,24,16),growth_rate=32,init_features=64,in_channels=input_m,middele_channels=128,classes=num_classes):
    ...
    def forward(self,x):
        ...
        x = x[:, fixed_indices, :]
        # x = x[:, :input_m, :]
        x = torch.Tensor.view(x, (-1, input_m, input_n)) # 输入数据转换m行n列
...
if __name__ == '__main__':
    input = torch.randn(size=(1,256,4))
    model = DenseNet(layer_num=(6,12,24,16),growth_rate=32,in_channels=4,classes=6)
    output = model(input)
    print(output.shape)
