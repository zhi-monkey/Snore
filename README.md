...
仅使用**单通道鼾声/呼吸音频**（500 Hz）预测 OSA 严重程度，不依赖 PSG 其他通道（SpO₂、口鼻气流、胸部呼吸运动等）。
...
原始 ResNet50 已支持通过构造参数调整，**无需修改网络结构**，只需修改以下参数：

| 参数          | 体位分类（原）  | OSA 严重程度分类（新）      |
|---------------|-----------------|-----------------------------|
| `in_channels` | 4               | **1**（单通道鼾声/呼吸音频）|
| `classes`     | 6               | **4**（4个严重程度）        |
| `dropout`     | 0.0（默认）     | **0.5**（防止过拟合）       |
...
# OSA 严重程度分类（新）
model = ResNet50(in_channels=1, classes=4, dropout=0.5)
```
...
| 文件                     | 作用                                                                 |
|--------------------------|----------------------------------------------------------------------|
| `build_dataset_osa.py`   | 从单通道鼾声音频构建带严重程度标签的数据集（`.npy` 格式）           |
| `dataset_osa.py`         | `OSASeverityDataset`：加载上述 `.npy` 文件供训练使用                |
| `train_osa_severity.py`  | 端到端训练脚本，使用 `ResNet50(in_channels=1, classes=4, dropout=0.5)` |

### 使用步骤

```bash
# 步骤 1：将各严重程度的鼾声数据按目录结构放好（详见 build_dataset_osa.py 注释）
# 步骤 2：构建数据集
